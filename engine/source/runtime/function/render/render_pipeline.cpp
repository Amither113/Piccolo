...
namespace Piccolo
{
    void RenderPipeline::initialize(RenderPipelineInitInfo init_info)
    {
        ...
        m_toon_pass               = std::make_shared<ToonPass>();
        ...
        m_toon_pass->setCommonInfo(pass_common_info);
        ...
        ToonPassInitInfo toon_init_info;
    ...
    void RenderPipeline::forwardRender(std::shared_ptr<RHI> rhi, std::shared_ptr<RenderResourceBase> render_resource)
    {
        ...
        ToonPass&         toon_pass          = *(static_cast<ToonPass*>(m_toon_pass.get()));
